...
```json5 title="path/to/identity.schema.json"
{
  // ...
  properties: {
    traits: {
...
- [Verification HTTP Route](https://github.com/zzpu/openuser-selfservice-ui-node/blob/master/src/routes/verify.ts)
- [Verification UI](https://github.com/zzpu/openuser-selfservice-ui-node/blob/master/views/verify.hbs)
...
1. An initial HTTP Request is made to
   ...
   supported as `:via`.
2. ORY Kratos redirects the browser to the URL set in
   `selfservice.flows.verification.ui_url` and appends the `request` URL Query
   Parameter (e.g. `https://example.org/verify?request=abcde`).
